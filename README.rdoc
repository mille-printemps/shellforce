...
A simple wrapper of Salesforce.com REST API, inspired by Rest Client developed by Adam Wiggins and Julien Kirch. 
...
By default, the shell sets the root resource path to <URL of your Salesforce.com organization> + '/services/data' + <the latest API version>, e.g. 'https://na7.salesforce.com/services/data/v23.0'. Specify the rest of the resource path to `get`, `post`, `patch` and `delete`. For SOQL and SOSL queries, just specify a query string after `query` or `search`, respectively. 
  ...
  >> patch '/sobjects/account/001A000000nhLV6IAN', '{"BillingCity" : "Tokyo"}'  # The ID is different in your Salesforce.com organization
  ...
  >> post '/chatter/feeds/news/me/feed-items', {"text" => "hoge"}   # A Ruby style hash can be specified instead of message segments like above
  ...
  >> post '/chatter/feed-items/0D5A000000YeellKAB/likes', ''  # The second argument is an empty string
...
`to` command will allow you to change the root resource path.
  ...
  >> to :data
  >> "/services/data/v23.0"  # The root resource path is set to 'https://na7.salesforce.com/services/data/v23.0'

For Apex REST, execute 'to :apex'. 

  >> to :apex
  >> "/services/apexrest"  # Your own Apex REST serivice has to be implemented beforehand
...
As an applicaiton of shellforce, a clone of REST Explorer, part of Workbench (https://workbench.developerforce.com/login.php) is available. Install some gems first before using it.
